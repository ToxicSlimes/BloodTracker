...
- Экспорт анализа в Markdown формат для документирования
- Визуализация белковых фракций (электрофорез) в виде графика
...
- Каталог упражнений с интеграцией внешнего API
  ...
  - Автоматический маппинг упражнений на группы мышц
...
│   │   │   │       ├── modals.js          # Общие модальные окна
│   │   │   │       └── color-picker.js     # Кастомизация цвета и шрифта
│   │   │   │   └── effects/               # Визуальные эффекты
│   │   │   │       ├── ascii-art.js       # ASCII-арт элементы
│   │   │   │       ├── sparks.js          # Анимация искр
│   │   │   │       ├── matrix-runes.js    # Matrix-руны эффект
│   │   │   │       └── progress-bar.js    # Прогресс-бар
│   │   │   └── assets/                     # Ресурсы
│   │   │       └── fonts/                 # Шрифты
...
- **Автоматическое создание** — база данных создаётся автоматически при первом запуске приложения
- **Shared connection** — все репозитории используют один экземпляр БД через `Connection=shared`
- **Автоматические индексы** — создаются при первом запуске в конструкторе `BloodTrackerDbContext`
- **Транзакции** — не поддерживаются между коллекциями (ограничение LiteDB)
- **Миграции** — не требуются, схема определяется классами доменной модели
- **Резервное копирование** — просто скопируйте файл `bloodtracker.db` в безопасное место
  - Рекомендуется делать резервные копии перед обновлением приложения
  - Файл можно перемещать между установками приложения
...
| Метод | Endpoint | Описание |
|-------|----------|----------|
| `GET` | `/api/exercisecatalog?muscleGroup={group}&search={text}` | Получить каталог упражнений (с фильтрацией по группе мышц и поиском) |

**Параметры запроса:**
- `muscleGroup` (опционально) — фильтр по группе мышц (0-11, см. enum MuscleGroup)
- `search` (опционально) — поиск по названию, части тела, целевой мышце или оборудованию

**Особенности:**
- Каталог кэшируется в базе данных на 24 часа
- При отсутствии API ключа возвращаются только кэшированные данные
- Автоматический маппинг упражнений на группы мышц
...
Используется для мониторинга состояния приложения и проверки доступности API.
...
```json
{
  ...
  "Database": {
    "ConnectionString": "Filename=bloodtracker.db;Connection=shared"
  },
  "Gemini": {
...
- **Database.ConnectionString** — строка подключения к LiteDB (по умолчанию: `Filename=bloodtracker.db;Connection=shared`)
  - Файл базы данных создаётся автоматически в папке приложения
  - `Connection=shared` позволяет использовать одну БД из нескольких репозиториев
- **Gemini.ApiKey** — API ключ для Gemini Vision API (опционально)
- **ExerciseCatalog.ApiKey** — API ключ для RapidAPI ExerciseDB (опционально)
- **ExerciseCatalog.ApiUrl** — URL API каталога упражнений
- **Serilog** — настройки логирования (консоль и файлы)
- **AllowedHosts** — разрешённые хосты для ASP.NET Core (по умолчанию: `*`)
...
```bash
set GEMINI__APIKEY=ваш-ключ
set EXERCISECATALOG__APIKEY=ваш-ключ
set EXERCISECATALOG__APIURL=https://exercisedb.p.rapidapi.com/exercises
set DATABASE__CONNECTIONSTRING=Filename=bloodtracker.db;Connection=shared
```

**Примечание:** В .NET переменные окружения используют двойное подчёркивание `__` для разделения вложенных секций (например, `Gemini:ApiKey` → `GEMINI__APIKEY`).
...
Это создаст файл `electron.manifest.json` с настройками Electron.
...
1. **Создаёт базу данных** — файл `bloodtracker.db` в папке приложения
2. **Создаёт индексы** — все необходимые индексы для быстрого поиска
3. **Загружает Tesseract данные** — языковые данные для OCR (при первом использовании импорта PDF)
4. **Инициализирует каталог упражнений** — загружает упражнения из внешнего API (если настроен API ключ)

**Примечание:** При первом использовании импорта PDF может потребоваться интернет-соединение для загрузки языковых данных Tesseract (~10-20 МБ).
...
**Примечание:** Без API ключа каталог будет использовать только кэшированные данные (если есть). Кэш обновляется автоматически каждые 24 часа при наличии API ключа и активного интернет-соединения.

## 🚀 Запуск приложения

Приложение поддерживает два режима работы:

1. **Electron режим** — десктопное приложение с встроенным браузером
2. **Браузер режим** — обычный веб-сервер, доступный через браузер

Приложение автоматически определяет режим работы:
- Если запущено через `electronize start` — работает в Electron режиме
- Если запущено через `dotnet run` — работает в браузер режиме
...
**Параметры окна Electron:**
- Размер: 1400x900 пикселей
- Минимальный размер: 1000x700 пикселей
- Скрыта строка меню (AutoHideMenuBar)
- Безопасность: NodeIntegration=false, ContextIsolation=true
...
По умолчанию приложение использует порт **5000**. Чтобы изменить порт:
...
2. Через launchSettings.json или appsettings.json:
```json
{
  "Kestrel": {
    "Endpoints": {
...
**CORS:** Приложение использует политику `AllowAll` для CORS, что позволяет обращаться к API из любого источника (полезно для разработки).

## 🔁 Типичный сценарий работы

- **Шаг 1. Настройка курса**
  - Создать активный курс в разделе `Courses`
  - Добавить препараты, привязав их к курсу
  - Вести журнал приёма препаратов через `Intake Logs`
- **Шаг 2. Добавление анализов крови**
  - Импортировать PDF-отчёт из лаборатории или создать анализ вручную
  - Проверить автоматически подсвеченные отклонения от нормы
  - При необходимости добавить заметки к анализу
- **Шаг 3. Сравнение анализов**
  - На странице `Compare` выбрать два анализа (до/после)
  - Проанализировать процентные изменения показателей и статус значений
  - При необходимости экспортировать анализ в Markdown и сохранить в свои записи
- **Шаг 4. Планирование тренировок**
  - Создать программу тренировок и дни по дням недели
  - Добавить упражнения с указанием группы мышц
  - Для каждого упражнения задать подходы (повторения, вес, длительность)
  - Использовать ASCII-схему мышц для визуального контроля распределения нагрузки
- **Шаг 5. Monitoring на Dashboard**
  - Следить за активным курсом, последними анализами и алертами
  - Быстро переходить к разделам `Analyses`, `Courses`, `Compare`, `Workouts`
...
- `BloodTracker 1.0.0.exe` — исполняемый файл приложения (portable версия)
- `win-unpacked/` — распакованная версия для распространения

**Особенности сборки:**

- **Статические файлы** — встроены в сборку через EmbeddedFileProvider (работает в single-file режиме)
- **База данных** — создаётся автоматически при первом запуске в папке рядом с `.exe`
- **Логи** — сохраняются в папке `logs/` рядом с `.exe`
- **Tesseract данные** — загружаются автоматически в папку `tessdata/` при первом использовании OCR
...
- **CRT эффект** — ретро-стиль с эффектом старого монитора (scanlines, flicker, vignette, noise)
- **Кастомизация цвета** — выбор основного цвета интерфейса через цветовую палитру или hex-код
  - Настройки сохраняются в `localStorage` браузера
  - Доступны предустановленные цвета: зелёный, голубой, пурпурный, жёлтый, оранжевый, розовый, лайм, фиолетовый
  - Поддержка кастомных цветов через hex-код
- **Выбор шрифта** — переключение между шрифтами Rotasuningr и IBM MDA
  - Настройки сохраняются в `localStorage` браузера
  - Применяется ко всему интерфейсу

### Визуальные эффекты
- **ASCII-арт элементы** — декоративные ASCII-рисунки (скелет, факелы, группы мышц)
- **Анимация искр** — динамические искры при взаимодействии
- **Matrix-руны** — фоновые анимированные руны/символы
- **Прогресс-бар** — ASCII-стилизованный индикатор загрузки
- **Glitch эффекты** — визуальные искажения для акцентов
- **Bloom эффекты** — свечение текста и элементов
...
- **Графики** — визуализация изменений показателей во времени
  - График электрофореза белковых фракций
  - Сравнение анализов с процентными изменениями
- **Навигация** — переключение между страницами (Dashboard, Analyses, Courses, Compare, Workouts)
- **Экспорт в Markdown** — копирование анализа в формате Markdown для документирования
- **Фильтрация и поиск** — по датам, лабораториям, группам мышц, упражнениям
...
- **`main.js`** — точка входа, инициализация приложения, загрузка данных
- **`state.js`** — глобальное состояние приложения (анализы, препараты, курсы, тренировки)
- **`api.js`** — обёртки для Fetch API, централизованная работа с backend
- **`utils.js`** — утилиты (форматирование дат, определение статусов значений)
- **`config.js`** — конфигурация приложения

**Страницы (`pages/`):**
- `dashboard.js` — Dashboard с метриками и алертами
- `analyses.js` — Управление анализами, импорт PDF, экспорт Markdown
- `compare.js` — Сравнение анализов с процентными изменениями
- `course.js` — Управление курсами, препаратами, логами приёма
- `workouts.js` — Тренировочные программы, дни, упражнения, подходы

**Компоненты (`components/`):**
- `navigation.js` — Навигация между страницами
- `modals.js` — Общие модальные окна (анализы, препараты, логи)
- `workoutModals.js` — Модальные окна для тренировок
- `muscleAscii.js` — ASCII-арт визуализация групп мышц
- `color-picker.js` — Кастомизация цвета и шрифта интерфейса

**Эффекты (`effects/`):**
- `ascii-art.js` — Генерация ASCII-арт элементов (скелет, факелы)
- `sparks.js` — Анимация искр при взаимодействии
- `matrix-runes.js` — Фоновые анимированные руны/символы
- `progress-bar.js` — ASCII-стилизованный прогресс-бар

### Управление состоянием

Состояние хранится в централизованном объекте `state`:
- `analyses` — список анализов
- `drugs` — список препаратов
- `intakeLogs` — журнал приёма
- `referenceRanges` — референсные значения
- `currentCourse` — текущий активный курс
- `workoutPrograms`, `workoutDays`, `workoutExercises`, `workoutSets` — данные тренировок
- Флаги редактирования (`editingAnalysisId`, `editingDrugId`, и т.д.)

### API клиент

Все запросы к backend проходят через функцию `api()` из `api.js`, которая:
...
- **Tesseract:** Требует загрузки языковых данных при первом использовании (~10-20 МБ)
  - Данные загружаются автоматически в папку `tessdata/` рядом с исполняемым файлом
  - Поддерживаются языки: русский (rus) и английский (eng)
- **Gemini API:** Имеет лимиты на количество запросов (бесплатный tier)
- **LiteDB:** Не поддерживает транзакции между коллекциями
- **Exercise Catalog:** Требует внешний API ключ для обновления каталога (кэш работает без ключа)
- **PDF парсинг:** Точность зависит от качества PDF (сканированные документы могут быть менее точными)
- **UI эффекты:** CRT эффекты и анимации могут влиять на производительность на слабых системах
- **База данных:** Файл `bloodtracker.db` должен находиться в папке приложения (нельзя перемещать отдельно от `.exe` в portable версии)
...
1. **Остановите приложение** (закройте окно Electron)
2. **Скопируйте файл базы данных:**
   ```bash
   copy src\BloodTracker.Api\bloodtracker.db backup\bloodtracker_backup_YYYYMMDD.db
   ```

Или вручную скопируйте файл `bloodtracker.db` из папки приложения в безопасное место.
...
2. **Замените файл базы данных:**
   ```bash
   copy backup\bloodtracker_backup_YYYYMMDD.db src\BloodTracker.Api\bloodtracker.db
   ```
3. **Запустите приложение**

**Важно:** Убедитесь, что версия приложения совместима с форматом базы данных. При обновлении приложения рекомендуется создавать резервную копию перед обновлением.
