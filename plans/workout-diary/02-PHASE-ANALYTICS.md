...
### 11.5. Dashboard Quick Start Button

На дашборде (главная страница) в секции `.quick-actions` добавить кнопку быстрого старта тренировки:

```
┌─── DASHBOARD ───────────────────────────────────────────┐
│                                                         │
│  [ + АНАЛИЗ ]  [ + ПРИЁМ ]  [ ИМПОРТ PDF ]             │
│  [ ▶ НАЧАТЬ ПЯТНИЧНУЮ ТРЕНИРОВКУ ]   ← NEW!            │
│                                                         │
│  ... остальной дашборд ...                              │
│                                                         │
└─────────────────────────────────────────────────────────┘
...
**Логика:**
1. При рендере дашборда → проверить активную программу + текущий день недели
2. Если есть WorkoutDay на сегодня и не проведён на этой неделе → показать кнопку
3. Если сегодняшний уже проведён → предложить следующий непроведённый
4. Если нет программ → не показывать кнопку
5. Если есть активная тренировка → показать "ПРОДОЛЖИТЬ ТРЕНИРОВКУ" вместо старта

**Тексты для дней недели (Russian):**
```typescript
const DAY_LABELS: Record<number, string> = {
    0: 'ВОСКРЕСНУЮ',   // Sunday
    1: 'ПОНЕДЕЛЬНИЧНУЮ', // Monday
    2: 'ВТОРНИЧНУЮ',    // Tuesday
    3: 'СРЕДОВУЮ',      // Wednesday
    4: 'ЧЕТВЕРГОВУЮ',   // Thursday
    5: 'ПЯТНИЧНУЮ',     // Friday
    6: 'СУББОТНЮЮ'      // Saturday
}
// Формат: "НАЧАТЬ {DAY_LABEL} ТРЕНИРОВКУ"
// Пример: "НАЧАТЬ ПЯТНИЧНУЮ ТРЕНИРОВКУ"
```

**При клике:**
- Вызвать `POST /api/v1/workout-sessions/start` с нужным `sourceDayId`
- Навигация в таб ТРЕНИРОВКИ → под-таб ТРЕНИРОВКА (активная)
- Или если активная есть → навигация к ней

### 11.6. Swipe-пагинация упражнений (Carousel)

Вместо текущего вертикального списка упражнений с аккордеоном — **горизонтальная карусель**:
...
**Фичи:**
- Свайп влево/вправо на мобилке (touch events: touchstart/touchmove/touchend)
- Стрелки ◀ ▶ для десктопа (и тоже мобилки)
- Dots pagination внизу (● = завершено, ◉ = текущее, ○ = не начато)
- CSS `scroll-snap-type: x mandatory` + `overflow-x: scroll` (нативный swipe!)

**Auto-swipe после завершения упражнения:**
1. Пользователь завершает последний подход в упражнении
2. Через 3 секунды → автоматически свайпает на следующее незавершённое
3. Toast: "Следующее: Разведение гантелей" (3 сек)
4. Если пользователь свайпнул сам раньше — отменить автосвайп

**Реализация:**
```typescript
function onExerciseCompleted(exerciseIndex: number): void {
    const nextIncomplete = findNextIncompleteExercise(exerciseIndex)
    if (nextIncomplete === null) return

    const toastEl = toast.info(`Следующее: ${nextIncomplete.name}`, 3000)

    autoSwipeTimer = setTimeout(() => {
        scrollToExercise(nextIncomplete.index)
    }, 3000)
}

// CSS-based smooth scroll
function scrollToExercise(index: number): void {
    const container = document.querySelector('.exercise-carousel')
...
### 11.7. Чеклист для реализации

```
Frontend:
  □ Dashboard Quick Start button ("НАЧАТЬ ПЯТНИЧНУЮ ТРЕНИРОВКУ")
  ...
  □ Swipe-пагинация упражнений (CSS scroll-snap carousel)
  □ Auto-swipe на следующее упражнение через 3 сек после завершения
  □ Dots pagination (● завершено / ◉ текущее / ○ не начато)
  □ Стрелки ◀ ▶ для навигации между упражнениями
  □ "ПРОДОЛЖИТЬ ТРЕНИРОВКУ" на дашборде (если активная)
...
### 11.8. ASCII Wireframes — все экраны Phase 2B

---

#### WIREFRAME 1: Dashboard с Quick Start Button

```
╔══════════════════════════════════════════════════════════════════╗
║  [ ДАШБОРД ] [ КУРС ] [ АНАЛИЗЫ ] [ СРАВНЕНИЕ ] [ ТРЕНИРОВКИ ] ║
║  ^^^^active                                                      ║
╠══════════════════════════════════════════════════════════════════╣
║                                                                  ║
║  ┌──────────────────────────────────────────────────────────┐   ║
║  │ [ + АНАЛИЗ ]  [ + ПРИЁМ ]  [ ИМПОРТ PDF ]               │   ║
║  │                                                          │   ║
║  │ [ ▶ НАЧАТЬ ПЯТНИЧНУЮ ТРЕНИРОВКУ ]  ← gold accent btn    │   ║
║  │   Грудь + Трицепс · 5 упр. · ~45мин                     │   ║
║  └──────────────────────────────────────────────────────────┘   ║
║                                                                  ║
║  ┌── ПРОГРЕСС КУРСА ───────────────────────────────────────┐   ║
║  │          ╭─────╮                                         │   ║
...
**Вариант с активной тренировкой:**
```
║  ┌──────────────────────────────────────────────────────────┐   ║
║  │ [ + АНАЛИЗ ]  [ + ПРИЁМ ]  [ ИМПОРТ PDF ]               │   ║
║  │                                                          │   ║
║  │ [ ▶ ПРОДОЛЖИТЬ ТРЕНИРОВКУ — 23:45 ]  ← green pulse     │   ║
║  │   Пятница · Грудь + Трицепс · 3/5 упр.                  │   ║
║  └──────────────────────────────────────────────────────────┘   ║
```

**Вариант без программ:**
```
║  ┌──────────────────────────────────────────────────────────┐   ║
║  │ [ + АНАЛИЗ ]  [ + ПРИЁМ ]  [ ИМПОРТ PDF ]               │   ║
║  │                                                          │   ║
║  │  (кнопка тренировки не показывается)                     │   ║
║  └──────────────────────────────────────────────────────────┘   ║
```

---

#### WIREFRAME 2: Workout Hub — Под-таб "ТРЕНИРОВКА" (нет активной)
...
║  ║   5 упражнений · ~45 мин · 20 подходов                  ║    ║
║  ║                                                          ║    ║
║  ║   Жим лёжа (4×8) · Жим гантелей (4×10)                  ║    ║
║  ║   Разведение (3×12) · Французский жим (3×10)            ║    ║
...
║  Другие дни:                                                     ║
║  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐            ║
║  │ Пн — Спина   │ │ Ср — Ноги   │ │ Пт — Грудь ✓ │            ║
║  │ 6 упр · 50м  │ │ 5 упр · 55м │ │ (проведена)  │            ║
║  │ [ НАЧАТЬ ]   │ │ [ НАЧАТЬ ]   │ │  14 фев      │            ║
║  └──────────────┘ └──────────────┘ └──────────────┘            ║
║  ┌──────────────┐                                               ║
║  │ Вс — Руки   │                                               ║
║  │ 4 упр · 35м │                                               ║
...
#### WIREFRAME 3: Active Workout — Exercise Carousel (главный экран!)
...
║  ⏱ 23:45        Подходы: 8/20        Тоннаж: 2,480 кг          ║
║                                                                  ║
║  ┌──────────────────────────────────────────────────────────┐   ║
║  │                                                          │   ║
║  │   ◀  2/5  Жим гантелей лёжа                       ▶    │   ║
...
║  │   └────────┴──────────┴───────┴──────┴───────────────┘  │   ║
║  │                                                          │   ║
║  │   ᵍ = ghost (прошлый раз), серый текст                   │   ║
║  │                                                          │   ║
║  │   ┌──────────────────────────────────────────────────┐  │   ║
║  │   │              [ + ЕЩЁ ПОДХОД ]                    │  │   ║
║  │   └──────────────────────────────────────────────────┘  │   ║
...
#### WIREFRAME 4: Quick Set Logger (bottom sheet) с "What to Beat"
...
║  │                                                          │   ║
║  │   ┌── What to Beat ─────────────────────────────────┐   │   ║
║  │   │  ПРОШЛЫЙ РАЗ: 32.5кг × 8  (7 фев) ← тап!      │   │   ║
║  │   │  ПОБЕЙ ЭТО: 35кг×6 или 32.5кг×10               │   │   ║
║  │   └─────────────────────────────────────────────────┘   │   ║
║  │                                                          │   ║
║  │   ┌── Almost PR! ──────────────────────────────────┐    │   ║
║  │   │  ⚡ ПОЧТИ РЕКОРД! PR: 37.5кг. До него 2.5кг!  │    │   ║
║  │   └────────────────────────────────────────────────┘    │   ║
║  │                                                          │   ║
║  │   Вес (кг)                                               │   ║
║  │   [-5] [-2.5]  ┌───────────┐  [+2.5] [+5]              │   ║
...
#### WIREFRAME 5: PR Celebration Modal
...
║         ║                                           ║            ║
║         ║   ┌─ Максимальный вес ─────────────┐     ║            ║
║         ║   │  Было: 32.5кг → Стало: 35кг    │     ║            ║
║         ║   │  Улучшение: +7.7%               │     ║            ║
║         ║   └─────────────────────────────────┘     ║            ║
║         ║                                           ║            ║
║         ║   ┌─ Расчётный 1RM ────────────────┐     ║            ║
║         ║   │  Было: 43.3кг → Стало: 46.7кг  │     ║            ║
║         ║   │  Улучшение: +7.9%               │     ║            ║
║         ║   └─────────────────────────────────┘     ║            ║
║         ║                                           ║            ║
║         ║        ┌─────────────────────┐            ║            ║
...
#### WIREFRAME 6: Auto-swipe Toast (после завершения упражнения)

```
╔══════════════════════════════════════════════════════════════════╗
║                                                                  ║
║  ... активная тренировка, все подходы упражнения #2 done ...    ║
║                                                                  ║
║   ◀  2/5  Жим гантелей лёжа ✓                           ▶     ║
║                                                                  ║
║   Сет 1   30кг × 10   ✓ 🟢                                     ║
...
║                                                                  ║
║  ┌──────────────────────────────────────────────────────────┐   ║
║  │  ➡ Следующее: Разведение гантелей              3 сек ⏳ │   ║
║  └──────────────────────────────────────────────────────────┘   ║
║                                                                  ║
║  ... через 3 секунды автоматически свайпает →                   ║
║                                                                  ║
╚══════════════════════════════════════════════════════════════════╝
```
...
#### WIREFRAME 7: "Same as Last Set" [ = ] quick button
...
Кнопка [ = ]:
→ Копирует из Сет 2: weight=32.5, reps=8
→ Сразу POST /complete-set (без открытия Quick Set Logger!)
→ Toast: "32.5кг × 8 (как прошлый)" + [ОТМЕНИТЬ]
→ Vibration: [50, 30, 50]
→ Запускает Rest Timer
```

---

#### WIREFRAME 8: Workout Hub — Под-таб "ИСТОРИЯ"
...
║  ИСТОРИЯ ТРЕНИРОВОК                                              ║
║                                                                  ║
║  ┌── 14 фев, Пт — Грудь + Трицепс ─────────────────────────┐  ║
║  │                                                           │  ║
║  │  ⏱ 47 мин   Тоннаж: 3,240 кг   Объём: 156   Подходы: 20│  ║
║  │                                                           │  ║
║  │  ┌── Сравнение с предыдущей ──────────────────────────┐  │  ║
║  │  │  📈 Тоннаж: +180 кг   📈 Объём: +12   → Время: 0  │  │  ║
║  │  └────────────────────────────────────────────────────┘  │  ║
║  │                                                           │  ║
║  └───────────────────────────────────────────────────────────┘  ║
║                                                                  ║
║  ┌── 12 фев, Ср — Ноги ─────────────────────────────────────┐  ║
║  │                                                           │  ║
║  │  ⏱ 55 мин   Тоннаж: 4,560 кг   Объём: 184   Подходы: 24│  ║
║  │                                                           │  ║
║  └───────────────────────────────────────────────────────────┘  ║
║                                                                  ║
║  ┌── 10 фев, Пн — Спина ────────────────────────────────────┐  ║
...
#### WIREFRAME 9: Mobile Layout (320-375px) — Carousel
...
Thumb zone: [ПОДХОД] и [=] в
нижней 2/3 экрана (reachable)
Swipe: влево/вправо на карусели
```

---

#### WIREFRAME 10: "+ Добавить упражнение" Modal
...
║  │                                                          │   ║
║  │  Фильтры:                                                │   ║
║  │  [Все мышцы ▼]  [Все типы ▼]  [Всё оборудование ▼]     │   ║
║  │                                                          │   ║
║  │  ┌─────────────────────────────────────────────────┐    │   ║
║  │  │  Жим штанги лёжа          Chest · Barbell       │    │   ║
║  │  │  Bench Press               Compound · Strength   │    │   ║
║  │  ├─────────────────────────────────────────────────┤    │   ║
║  │  │  Жим гантелей лёжа        Chest · Dumbbell      │    │   ║
║  │  │  Dumbbell Bench Press      Compound · Strength   │    │   ║
║  │  ├─────────────────────────────────────────────────┤    │   ║
║  │  │  Жим на наклонной лавке   Chest · Barbell       │    │   ║
║  │  │  Incline Bench Press       Compound · Strength   │    │   ║
║  │  └─────────────────────────────────────────────────┘    │   ║
║  │                                                          │   ║
║  │  → Тап на упражнение = добавить с 3 пустыми подходами   │   ║
║  │                                                          │   ║
║  │   ┌──────────────────────────────────────────────────┐  │   ║
║  │   │                 [ ОТМЕНА ]                        │  │   ║
║  │   └──────────────────────────────────────────────────┘  │   ║
...
#### WIREFRAME 11: Rest Timer (уже есть, для полноты)

```
┌──────────────────────────────────────────────────────────────┐
│  ░░░░░░░░░ ОТДЫХ: 1:23 ░░░░░░░░░  [+30с] [Пропустить]     │
│  ████████████████████████░░░░░░░░░  progress bar            │
└──────────────────────────────────────────────────────────────┘
← fixed bottom bar, поверх всего UI
```

---

#### WIREFRAME 12: Exercise Carousel — Swipe States

**State A: Свайп происходит (анимация)**
```
│    ╱─────────────────╲    ╱─────────────────╲    │
│   │  Жим гантелей ← │   │ → Разведение     │   │
│   │  (уходит)        │   │   (приходит)      │   │
│    ╲─────────────────╱    ╲─────────────────╱    │
│                                                   │
│              ● ● ◉ ○ ○ → ● ● ● ◉ ○              │
...
**State B: Все подходы упражнения done → пульс обводки**
```
│   ┌══════════════════════════════════════════┐   │
│   ║  2/5  Жим гантелей лёжа  ✓ ГОТОВО      ║   │
│   ║                                          ║   │
│   ║  Сет 1   30кг × 10  ✓ 🟢               ║   │
│   ║  Сет 2   32.5кг × 8 ✓ 🟢               ║   │
│   ║  Сет 3   35кг × 8   ✓ 🟢               ║   │
│   ║  Сет 4   35кг × 6   ✓ 🟡               ║   │
│   ║                                          ║   │
│   ║  green border pulse → auto-swipe 3сек   ║   │
│   ╚══════════════════════════════════════════╝   │
```

**State C: Все упражнения done**
```
│   ╔══════════════════════════════════════════╗   │
│   ║                                          ║   │
│   ║        ✓ ВСЕ УПРАЖНЕНИЯ ГОТОВЫ!          ║   │
│   ║                                          ║   │
│   ║   5/5 упражнений · 20/20 подходов       ║   │
│   ║   Тоннаж: 4,240 кг · Время: 47:23      ║   │
│   ║                                          ║   │
│   ║   ┌──────────────────────────────────┐  ║   │
...
### 11.9. Приоритеты внутри Phase 2B

| # | Задача | Effort | Приоритет |
|---|--------|--------|-----------|
| 1 | Dashboard Quick Start button | 0.25 дня | P0 |
| 2 | Workout Hub (3 под-таба) | 0.5 дня | P0 |
| 3 | Smart Day Suggestion | 0.5 дня | P0 |
| 4 | Swipe-карусель упражнений + auto-swipe | 0.5 дня | P0 |
| 5 | "+ подход" / "+ упражнение" | 0.5 дня | P0 |
| 6 | "Same as Last Set" quick button [ = ] | 0.25 дня | P0 |
| 7 | Analytics Screen (charts) | 1 день | P1 |
| 8 | Calendar Heatmap | 0.5 дня | P1 |
| 9 | Stats Overview | 0.5 дня | P1 |

**P0 = UX flow (делаем первым!), P1 = analytics/charts (потом)**
**Estimated P0: ~2.5 дня, P1: ~2 дня**
